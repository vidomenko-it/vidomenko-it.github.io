...
Migration en direct.
## 1.1. Début. En lien avec la baisse du prix des cartes réseau (j'en ai acheté une paire à deux ports avec une paire de câbles en cuivre pour 100$ avec livraison), il y avait une volonté de construire un cluster de deux nœuds avec deux points de panne, connecté directement par trois réseaux filaires et un WiFi, devant un disque NVME pour le stockage partagé. C'était censé être une option de cluster pour la maison ou pour une petite entreprise. Le choix s'est porté sur le stockage drbd avec rdma.
