---
title: "2.	Про один з способів реалізації RAID1 спільного сховища для кластера Proxmox з двох вузлів з їх дисків."
categories:
  - Blog
tags:
  - DRBD 
...
## 4.1. Встановлюємо покети на кожному з вузлів: apt install open-iscsi multipath-tools .  

**Запускаємо сервіс:**  

`systemctl start open-iscsi.service`  

### 4.2. На першому вузлі підєднуємось до віртуальної машини, ми знаходимо ціль і входимо в систему: : iscsiadm -m discovery -t st -p 10.10.1.3  
...
## 6. В графічній оболонці Datacenter Storage додаємо iSCSI і далі LVM, ставимо позначку Shared і обираємо відповідно вузли.
...
### 7.1.  1. У файлі /usr/share/initramfs-tools/scripts/local шукаємо на рядках 179-185 (попередньо зробивши бекап файлу):  
```
...
```
   #checkfs "${ROOT}" root "${FSTYPE}"
	...
	umount /ramboottmp
```
### 2. Зберігаємо файл. І вводимо команду в терміналі від рута:  

`mkinitramfs -o /boot/initrd.img-ramboot`  

### 3. Перевіряємо, що файл створено в папці /boot і повертаємо старий local у папці /usr/share/initramfs-tools/scripts/local на місце (або видаляємо всі наші зміни, які ми зробили за крок 1).  

### 4. Ідемо в папку / etc і знаходимо файл fstab, зберігаємо його копію і редагуємо його, шукаємо в перших рядках щось на зразок цього:  

`UUID= d0ed981c-d33f-49e2-ae4f-0b28f4d11d6a / ext4 defaults 1 1`

**і міняємо на:**  

`none / tmpfs defaults 0 0` 

### 5. Зберігаємо та йдемо в папку /boot/grub, знаходимо grub.cfg та змінюємо (попередньо зберігши копію) ці рядки 106-121:  
```
function gfxmode {
        set gfxpayload="${1}"
}
...
menuentry 'Debian GNU/Linux' --class debian --class gnu-linux --class gnu --class os $menuentry_id_option 'gnulinux-simple-d0ed981c-d33f-49e2-ae4f-0b28f4d11d6a' {
        load_video
        insmod gzio
        ...
        if [ x$feature_platform_search_hint = xy ]; then
          search --no-floppy --fs-uuid --set=root --hint-bios=hd0,msdos1 --hint-efi=hd0,msdos1 --hint-baremetal=ahci0,msdos1  d0ed981c-d33f-49e2-ae4f-0b28f4d11d6a
        else
          search --no-floppy --fs-uuid --set=root d0ed981c-d33f-49e2-ae4f-0b28f4d11d6a
        fi
        echo    'Loading Linux 6.1.0-28-amd64 ...'
        linux   /boot/vmlinuz-6.1.0-28-amd64 root=UUID=d0ed981c-d33f-49e2-ae4f-0b28f4d11d6a ro initrd=/boot/ram.tar.gz quiet splash toram
        echo    'Loading initial ramdisk ...'
        initrd  /boot/initrd.img-6.1.0-28-amd64
}
```
**на той код, що знаходиться нижче, тобто просто додаємо ще одне меню, трохи видозмінене в назві і на рядку initrd (для інших систем, цей код меню, природно, буде інший, але ті зміни, які зробили ми, будуть ті ж самі най):**
```
function gfxmode {
        set gfxpayload="${1}"
...
menuentry 'RAMDebian GNU/Linux' --class debian --class gnu-linux --class gnu --class os $menuentry_id_option 'gnulinux-simple-d0ed981c-d33f-49e2-ae4f-0b28f4d11d6a' {
        load_video
        insmod gzio
        if [ x$grub_platform = xxen ]; then insmod xzio; insmod lzopio; fi
        insmod part_msdos
...
```
### 7.2. Тепер зробимо ram.tar.gz, вимкнувши віртуальну машину, завантажуємо нову віртуальну машину в режимі liveCD, підключивши диск цієї віртуальної машини.  
...
```
### 7.3. Тепер при загрузці віртуальної машини, вибираємо відповідне меню завантаження в RAM. Після завантаження відімкнемо диск:  

#### 7.3.1. Зупинимо доступ до диска:  
...
lsblk
```
#### 7.3.4. Від’єднання диска від віртуальної машини через QEMU монітор:  

##### 7.3.4.1. Увійдіть у монітор QEMU для конкретної VM:  
